...
export default {
    ...
    methods:{
        ...
        getData(){
            const father = this
            this.axios.get('http://127.0.0.1:8000/api/course/allcourse/').then(function(response){
                console.log(response.data)
                father.list = response.data
                father.dataCount = response.data.allcourse.length
                console.log(father.dataCount)
                if(father.dataCount<father.pageSize){
                    father.course = response.data.allcourse
                }else{
                    father.course = response.data.allcourse.slice(0,father.pageSize)
    ...
    changepage(index){
        ...
        father.course = father.list.allcourse.slice(_start,_end)
    },
    collection(id){
