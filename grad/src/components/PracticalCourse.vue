<template>
    <div class="freecourse">
        <PracticalCourseNav></PracticalCourseNav>
...
<script>
import PracticalCourseNav from '@/components/header/PracticalCourseNav'
export default {
    data(){
        return {
            // 全部课程
    ...
    methods:{
        ...
        getData(){
            const father = this
            this.axios.get('http://127.0.0.1:8000/api/course/practicalcourse/').then(function(response){
                console.log(response.data)
                father.list = response.data
                father.dataCount = response.data.practicalcourse.length
                console.log(father.dataCount)
                if(father.dataCount<father.pageSize){
                    father.course = response.data.practicalcourse
                }else{
                    father.course = response.data.practicalcourse.slice(0,father.pageSize)
