    ...
    <div class="freecourse">
        ...
        <div class="course_list">
            ...
            <div class="list" v-for="(item,index) in course" :key="index" >
                ...
                <div class="collect" @click="collection(item.id)" style="cursor:pointer">
                   收藏                  
                    </div>
...
<script>
import AllCourseNav from '@/components/header/AllCourseNav'
export default {
    data(){
        return {
            // 全部课程
    ...
    methods:{
        getData(){
            ...
            this.axios.get('http://127.0.0.1:8000/api/course/selectcourse/',{params:{tag:this.$route.query.tag}}).then(function(response){
                console.log(response.data)
                father.list = response.data
                father.dataCount = response.data.selectcourse.length
                console.log(father.dataCount)
                
                if(father.dataCount<father.pageSize){
                    father.course = response.data.selectcourse
                }else{
                    father.course = response.data.selectcourse.slice(0,father.pageSize)
    ...
    changepage(index){
        ...
        father.course = father.list.selectcourse.slice(_start,_end)
    },
    change(){
