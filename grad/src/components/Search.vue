...
export default {
    ...
    methods:{
        ...
        getData(){
            const father = this
            this.axios.get('http://127.0.0.1:8000/api/course/searchcourse/',{params:{searchData:this.$route.query.searchData}}).then(function(response){
                console.log(response.data)
                father.list = response.data
                father.dataCount = response.data.searchcourse.length
                console.log(father.dataCount)
                if(father.dataCount<father.pageSize){
                    father.course = response.data.searchcourse
                }else{
                    father.course = response.data.searchcourse.slice(0,father.pageSize)
    ...
    changepage(index){
        ...
        father.course = father.list.searchcourse.slice(_start,_end)
    },
    collection(id){
